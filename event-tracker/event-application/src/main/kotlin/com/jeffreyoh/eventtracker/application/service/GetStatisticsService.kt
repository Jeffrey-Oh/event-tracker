...
class GetStatisticsService(
    ...
    override fun getCount(componentId: Long, eventType: EventType): Mono<Long> {
        return when(eventType) {
            EventType.CLICK -> statisticsRedisPort.getEventCount(eventType, EventCommand.EventMetadata(componentId = componentId))
            EventType.PAGE_VIEW -> statisticsRedisPort.getEventCount(eventType, EventCommand.EventMetadata(componentId = componentId))
            EventType.SEARCH -> statisticsRedisPort.getEventCount(eventType, EventCommand.EventMetadata(componentId = componentId))
    ...
    override fun getLikeCount(componentId: Long, postId: Long): Mono<Long> {
        return statisticsRedisPort.getEventCount(EventType.LIKE, EventCommand.EventMetadata(componentId = componentId, postId = postId))
    }
